...
def output(model, model_path, input_path, output_path, asciiMapping):
    model = model()
    model.load_state_dict(torch.load(model_path))
    model = model.eval()
    ...
    for f in files:
        ...
        if i % 100 == 0:
            #print("input")
            join_input(example, input_path + 'textfiles/', "input" + str(i), asciiMapping)
            #print("output")
            join_output_deterministic(output, asciiMapping)
        i += 1
